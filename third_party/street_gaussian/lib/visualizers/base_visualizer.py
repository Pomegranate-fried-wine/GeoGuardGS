...
class BaseVisualizer():
    ...
    def visualize_depth(self, result, camera: Camera):
        ...
        if acc is not None:
            depth = raw_depth / (acc + 1e-10)
        else:
            depth = raw_depth

        # Save expected depth for LiDAR evaluation; keep raw accumulation and alpha for diagnostics.
        depth_np = depth.detach().cpu().numpy().squeeze()
        np.save(os.path.join(self.result_dir, f'{name}_depth.npy'), depth_np)
        np.save(os.path.join(self.result_dir, f'{name}_raw_depth.npy'), raw_depth.detach().cpu().numpy().squeeze())
        if acc is not None:
            np.save(os.path.join(self.result_dir, f'{name}_acc.npy'), acc.detach().cpu().numpy().squeeze())
