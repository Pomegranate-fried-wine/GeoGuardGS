...
class StreetGaussianRenderer():
    ...
    def render_kernel(
        ...
        if features is not None:
            rasterizer_kwargs["semantics"] = features

        rasterizer_result = rasterizer(**rasterizer_kwargs)
        if len(rasterizer_result) == 5:
            rendered_color, radii, rendered_depth, rendered_acc, rendered_feature = rasterizer_result
        else:
            rendered_color, radii, rendered_depth, rendered_acc = rasterizer_result
            rendered_feature = torch.empty(0, *rendered_color.shape[1:], device=rendered_color.device)
