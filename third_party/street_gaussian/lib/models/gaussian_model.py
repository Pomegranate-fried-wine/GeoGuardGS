...
def initial_dist2(points):
    try:
        from simple_knn._C import distCUDA2
        return distCUDA2(points)
    except ImportError as exc:
        print(f"[WARN] simple_knn unavailable; using constant Gaussian scale fallback: {exc}")
        return torch.full((points.shape[0],), 0.01, dtype=points.dtype, device=points.device)
...
class GaussianModel(nn.Module):
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        dist2 = torch.clamp_min(initial_dist2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001)
    ...
    def training_setup(self):
        ...
        if (
            not hasattr(self, "max_radii2D")
            or self.max_radii2D is None
            or self.max_radii2D.shape[0] != self.get_xyz.shape[0]
        ):
            self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
