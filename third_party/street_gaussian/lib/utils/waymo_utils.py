...
# 更加稳妥的写法，直接根据下划线分割
image_filename_to_cam = lambda x: int(x.split('.')[0].split('_')[-1]) if '_' in x else -1
...
def get_obj_pose_tracking(datadir, selected_frames, ego_poses, cameras=[0, 1, 2, 3, 4]):
    ...
    print(f"DEBUG: image_dir path is {os.path.abspath(image_dir)}") # 加上这行
...
def generate_dataparser_outputs(
    ...
    print(f"DEBUG >>> Total images found: {len(image_filenames_all)}")

    # 2. 修改计算逻辑：不要死磕 // 5，因为 995 张图可能每帧不是正好 5 张
    # 我们直接通过解析文件名获取最大帧号
    all_frame_indices = [image_filename_to_frame(os.path.basename(f)) for f in image_filenames_all]
    num_frames_all = max(all_frame_indices) + 1 if all_frame_indices else 0
    print(f"DEBUG >>> Max frame index + 1 = num_frames_all: {num_frames_all}")
    ...
    # 这一行输出非常重要，必须看到它不是 0 to -1
    print(f"DEBUG >>> Final range being used: {start_frame} to {end_frame}")
    ...
    cams_timestamps = []
    ##
    print(f"DEBUG: range is {start_frame} to {end_frame}")
    ##
    ...
    if use_colmap and not os.path.exists(os.path.join(colmap_basedir, 'triangulated/sparse/model')):
